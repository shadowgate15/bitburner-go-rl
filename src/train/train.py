...
An extended curriculum training loop is available via
:func:`train_with_curriculum` which integrates
:class:`~src.league.curriculum.BoardCurriculum`,
:class:`~src.league.opponents.OpponentPool`, and
...
class CurriculumTrainConfig(TrainConfig):
    """Extended training configuration for curriculum + league training.

    Inherits all fields from :class:`TrainConfig` and adds parameters
    that control the curriculum scheduler, opponent pool, and
    evaluation cadence.

    Attributes:
        eval_interval: Run a full evaluation pass every N iterations.
        league_interval: Add the current checkpoint to the league
            opponent pool every N iterations.
        num_eval_games: Number of games per opponent during evaluation.
        max_historical_opponents: Maximum historical checkpoints kept
            in the opponent pool.
        transition_mix: Probability of sampling the *current* board
            size during a curriculum transition (see
            :class:`~src.league.curriculum.BoardCurriculum`).
        best_metric_key: Metric key used by
            :class:`~src.league.checkpoint.CheckpointManager` to
            decide whether to overwrite ``best.pt``.
    """

    eval_interval: int = 50
    league_interval: int = 20
    num_eval_games: int = 50
    max_historical_opponents: int = 20
...
def train_with_curriculum(
    ...
    """Run a PPO training loop with curriculum learning and league self-play.

    Extends the standard :func:`train` loop with:

    1. A :class:`~src.league.curriculum.BoardCurriculum` that
       advances the active board size when win-rate thresholds are
       met against built-in IPvGO bots.
    2. An :class:`~src.league.opponents.OpponentPool` that mixes
       built-in bots, random play, and historical self-play checkpoints
       according to the curriculum stage.
    3. A :class:`~src.league.checkpoint.CheckpointManager` that
       saves ``latest.pt``, ``best.pt``, and league snapshots.
    4. Periodic evaluation via
       :func:`~src.league.evaluation.evaluate` to measure win rates
       against every opponent category.

    Each training iteration:

    1. Query the curriculum for the current board size and stage.
    2. Sample an opponent from the pool (stage-aware distribution).
    3. Log the sampled opponent type.
    4. Run on-policy PPO data collection (``SyncDataCollector``).
    5. Compute GAE, normalise advantages, PPO update.
    6. Log metrics.

    Every *eval_interval* iterations:

    * Evaluate the agent against all opponent categories.
    * Update the curriculum with the new metrics.
    * Save ``latest.pt`` and conditionally ``best.pt``.

    Every *league_interval* iterations:

    * Save a permanent league snapshot and register it with the pool.

    Args:
        cfg: Curriculum training configuration.  Defaults to
            ``CurriculumTrainConfig()`` (all defaults).

    Note:
        Evaluation against built-in bots will raise
        :exc:`NotImplementedError` until the server-side
        ``builtin_move`` WebSocket API is implemented.  The rest of
        the training loop is unaffected.
    """
    if cfg is None:
        cfg = CurriculumTrainConfig()

    # ---- imports (deferred to avoid circular dependency at module level)
    ...
    # ------------------------------------------------------------------
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"[curriculum] Using device: {device}")

    # ------------------------------------------------------------------
    # 2. Curriculum, opponent pool, checkpoint manager
    # ------------------------------------------------------------------
    curriculum = BoardCurriculum(transition_mix=cfg.transition_mix)
    opponent_pool = OpponentPool(
        websocket_uri=cfg.websocket_uri,
        max_historical=cfg.max_historical_opponents,
        board_size=curriculum.current_size,
    )
    ckpt_manager = CheckpointManager(
        checkpoint_dir=cfg.checkpoint_dir,
        best_metric_key=cfg.best_metric_key,
    )

    print(
        f"[curriculum] Starting at board size {curriculum.current_size}, "
        f"stage={curriculum.get_stage()}"
    )

    # ------------------------------------------------------------------
    # 3. Build networks for the initial board size
    # ------------------------------------------------------------------
    active_size = curriculum.current_size
    active_cfg = CurriculumTrainConfig(
        **{
            k: v
            for k, v in cfg.__dict__.items()
            if k != "_extra" and not k.startswith("_")
        }
    )
    active_cfg.board_size = active_size

    actor, critic = build_network(active_cfg, device)
    opponent_pool.set_latest_actor(actor)

    print(
        f"[curriculum] Networks built for {active_size}x{active_size}."
    )

    # ------------------------------------------------------------------
    # 4. GAE, PPO loss, optimiser
    # ------------------------------------------------------------------
    advantage_module = GAE(
        gamma=cfg.gamma,
        lmbda=cfg.lmbda,
    ...
    def _make_env() -> TorchRLGoEnv:
        return make_env(board_size=active_size, websocket_uri=uri)
    ...
    print(
        f"[curriculum] Training: {total_iters} iterations, "
        f"{cfg.total_frames:,} total frames"
    )
    ...
    for data in collector:
        ...
        # ------ curriculum stage / opponent sampling ------------------
        board_size_this_iter = curriculum.get_board_size()
        stage = curriculum.get_stage()
        sampled_opp = opponent_pool.sample_opponent(stage)
        opp_type = type(sampled_opp).__name__
        ...
        if iter_idx % cfg.log_interval == 0:
            ...
            frames = iter_idx * cfg.frames_per_batch
            fps = frames / elapsed
            print(
                f"[iter {iter_idx:4d}/{total_iters}] "
                f"board={board_size_this_iter}x{board_size_this_iter} "
                f"stage={stage} "
                f"opp={opp_type} "
                f"frames={frames:7,} fps={fps:6.0f} "
                f"reward={mean_reward:+.4f} loss={mean_loss:.4f}"
            )

        # ------ evaluation + curriculum update -----------------------
        if iter_idx % cfg.eval_interval == 0:
            print(f"[curriculum] Evaluating at iteration {iter_idx}…")
            try:
                ...
                print(
                    f"[curriculum] Board={curriculum.current_size} "
                    f"stage={curriculum.get_stage()} "
                    + " ".join(
                        f"{k}={v:.3f}"
                ...
                ckpt_manager.save_latest(
                    actor, critic, optimizer, iter_idx,
                    active_cfg, curriculum_state,
                )
                ckpt_manager.maybe_save_best(
                    eval_metrics, actor, critic, optimizer,
                    iter_idx, active_cfg, curriculum_state,
                )
            except NotImplementedError as exc:
                print(f"[curriculum] Evaluation skipped: {exc}")
                curriculum_state = curriculum.state_dict()
                ckpt_manager.save_latest(
                    actor, critic, optimizer, iter_idx,
                    active_cfg, curriculum_state,
        ...
        # ------ league checkpoint ------------------------------------
        if iter_idx % cfg.league_interval == 0:
            league_path = ckpt_manager.add_to_league(
                actor, critic, iter_idx, active_cfg,
                curriculum.state_dict(),
            )
            opponent_pool.add_checkpoint(league_path)
            opponent_pool.set_latest_actor(actor)
            print(
                f"[curriculum] League snapshot → {league_path}"
            )

    # ------ final save -----------------------------------------------
    final_path = ckpt_manager.save_latest(
        actor, critic, optimizer, iter_idx, active_cfg,
        curriculum.state_dict(),
    )
    print(f"[curriculum] Training complete. Final checkpoint → {final_path}")
    collector.shutdown()
